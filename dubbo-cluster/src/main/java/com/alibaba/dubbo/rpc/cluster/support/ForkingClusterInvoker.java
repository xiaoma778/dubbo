                ...
                executor.execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            // 原因是这样的，在并行调用多个服务提供者的情况下，只要有一个
                            // 服务提供者能够成功返回结果，而其他全部失败。此时
                            // ForkingClusterInvoker 仍应该返回成功的结果，而非抛出
                            // 异常。在value >= selected.size()时将异常对象放入阻塞
                            // 队列中，可以保证异常对象不会出现在正常结果的前面，这样可从
                            // 阻塞队列中优先取出正常的结果。
