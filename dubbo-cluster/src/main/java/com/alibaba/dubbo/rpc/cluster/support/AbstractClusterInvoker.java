...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        boolean sticky = invokers.get(0).getUrl().getMethodParameter(methodName, Constants.CLUSTER_STICKY_KEY, Constants.DEFAULT_CLUSTER_STICKY);
        {
            ...
            // stickyInvoker，表明 stickyInvoker 对应的服务提供者可能因网络原因未能成功提供服务（因为 selected 中有值就表示之前已经调用过且失败了）。
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        } else { // do not check invoker.isAvailable()  不检查 Invoker 可用性
            for (Invoker<T> invoker : invokers) {
                // 如果 selected 列表不包含当前 invoker，则将其添加到 reselectInvokers 中
                if (selected == null || !selected.contains(invoker)) {
                    reselectInvokers.add(invoker);
    ...
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        List<Invoker<T>> invokers = list(invocation);//服务提供者可能会有多个，这里根据 invocation 信息获取到对应的多个服务提供者
        if (invokers != null && !invokers.isEmpty()) {
