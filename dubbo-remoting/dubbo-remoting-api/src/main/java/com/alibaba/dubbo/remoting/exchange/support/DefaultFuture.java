...
public class DefaultFuture implements ResponseFuture {
    ...
    static {
        // 启动一个守护线程，每隔30毫秒检测 FUTURES 中是否有超时的 DefaultFuture
    ...
    public Object get(int timeout) throws RemotingException {
        ...
        if (!isDone()) {
            ...
            try {
                ...
                while (!isDone()) {
                    // 如果调用结果尚未返回，这里等待一段时间【注：done 是通过 ReentryLock.newCondition() 创建出来的，当调用 await() 方法时会释放锁（其底层就是 aqs 的等待队列了）】
