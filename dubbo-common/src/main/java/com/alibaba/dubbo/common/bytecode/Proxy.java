...
public abstract class Proxy {
    ...
    /**
     * Get proxy.
     * 该方法代码比较复杂，我们写了大量的注释。大家在阅读这段代码时，要搞清楚 ccp 和 ccm 的用途，不然会被搞晕。
     ...
     * 下面以 org.apache.dubbo.demo.DemoService 这个接口为例，来看一下该接口代理类代码大致是怎样的（忽略 EchoService 接口）：
        ...
        try {
            ...
            for (int i = 0; i < ics.length; i++) {
                ...
                for (Method method : ics[i].getMethods()) {
                    ...
                    for (int j = 0; j < pts.length; j++)
                        code.append(" args[").append(j).append("] = ($w)$").append(j + 1).append(";");// 生成 args[1...N] = ($w)$1...N;
            ...
            synchronized (cache) {
                if (proxy == null)
                    ...
                    cache.put(key, new WeakReference<Proxy>(proxy));// 写缓存
                cache.notifyAll();// 唤醒其他等待线程
            }
        }
        return proxy;
    }
